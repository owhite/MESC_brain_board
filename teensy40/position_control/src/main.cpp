...
void loop() {
  ...
  if (g_button.hasChanged()) {
    ...
    if (pb_state == PB_PRESSED) {
      ...
      if (supervisor.mode == test_mode) {
        ...
        supervisor.user_test_can_total_us = BALANCE_BUTTON_RUN_US;
  ...
  if (now_us - last_lowprio_us >= (CONTROL_PERIOD_US * 100)) {
    ...
    if (line == "end" || line == "idle") {
      supervisor.mode = SUP_MODE_IDLE;
      Serial.println("serial: entering idle mode");
    } else if (line == "run") {
		  Serial.println("serial: received run command; starting test_can_transmit mode");
		  tone_start(&g_tone, PB_BEEP_HZ, PB_BEEP_MS, PB_GAP_MS);
      supervisor.user_test_can_total_us = BALANCE_BUTTON_RUN_US;
		  supervisor.mode = SUP_MODE_TEST_CAN;
    } else if (line == "pwm_control") {
      supervisor.mode = SUP_MODE_PWM_POS_CONTROL;
      tone_start(&g_tone, PB_BEEP_HZ, PB_BEEP_MS, PB_GAP_MS);
      Serial.println("serial: starting pwm_pos_control mode");
    } else if (line.startsWith("timed_sweep")) {
		  int deg = 0;
      if (sscanf(line.c_str(), "timed_sweep %d", &deg) == 1) {
		    if (deg < -10000 || deg > 10000) {
          Serial.println("serial: timed_sweep out of range (use -10000..10000)");
		    } else {
          supervisor.user_timed_pos_sweep_deg = deg;
          supervisor.user_timed_pos_total_us = BALANCE_BUTTON_RUN_US;
          supervisor.mode = SUP_MODE_TIMED_POS_CONTROL;
		      tone_start(&g_tone, PB_BEEP_HZ, PB_BEEP_MS, PB_GAP_MS);
          Serial.printf("serial: starting timed_pos_control mode with timed_sweep %d deg\r\n", deg);
		    }
		  } else {
        Serial.println("serial: usage: timed_sweep <deg> (deg in -10000..10000)");
		  }
		}
		input = "";  // reset buffer
	      } else {
