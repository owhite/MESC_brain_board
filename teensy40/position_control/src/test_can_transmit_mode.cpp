...
void test_can_transmit_mode(Supervisor_typedef *sup,
  ...
  if (++report_counter >= TELEMETRY_DECIMATE) {
    ...
    const int32_t dt_err_from_1khz_us = (int32_t)loop_dt_us - 1000;
    const int dt_ok_1khz = (abs(dt_err_from_1khz_us) <= 100) ? 1 : 0;
    ...
    Serial.printf(
        ...
        "\"loop_dt_us\":%lu,\"loop_hz\":%.2f,\"dt_err_1khz_us\":%ld,\"dt_ok_1khz\":%d,"
        "\"exec_us\":%lu,\"ovr\":%lu}\r\n",
        micros(),
        pos_L,
        pos_R,
  ...
  if (sup->user_test_can_total_us > 0 && elapsed_us > sup->user_test_can_total_us) {
#ifdef SEND_TORQUE
    ...
    Serial.printf("balance exit: timed stop -> idle (elapsed=%lu us, limit=%lu us)\r\n",
                  ...
                  (unsigned long)sup->user_test_can_total_us);
    sup->mode = SUP_MODE_IDLE;
    first_entry = true;
    last_posvel_rx_used_L_us = 0;
    last_posvel_rx_used_R_us = 0;
