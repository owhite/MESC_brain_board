...
void loop() {
  ...
  if (now_us - last_lowprio_us >= (CONTROL_PERIOD_US * 100)) {
	...
	if (!err) {
	  ...
	  if (doc.containsKey("cmd") && doc["cmd"] == "run_position") {
	    /* 
	    ...
	    */ 
	    ...
	    if (supervisor.mode == SUP_MODE_POSITION_CONTROL) {
	      supervisor.mode = SUP_MODE_IDLE;
	    }
	    else {
	      supervisor.mode = SUP_MODE_POSITION_CONTROL;
	    }
	  }
	}
	input = "";  // reset buffer
	...
	SupervisorMode test_mode = SUP_MODE_POSITION_CONTROL;
